<?php 
	...
	if ($rs['active'] =='0')	
	{
		echo "<script>alert('อยู่ระหว่างตรวจสอบข้อมูลการสมัคร!'); window.location ='index.php?page=home';</script>";		
	}
                ...
                <div class="row" >
                    <div class="col-sm-12">
						<form enctype="multipart/form-data" class="form-horizontal" role="form" name="formregister" method="post" action="booking\action.php?action=add">
									...
									<select name="idrooms" id="idrooms" >
										<?php 
											...
											<option value="<?php echo $meResult['id_rooms'];?>">
															...
															( รองรับ
															<?php echo $meResult['people_rooms'];?>
															คน )
															</option>
										<?php } ?>
									</select>
							...
							while ($meResult = $meQuery->fetch_assoc()){
							?> 	  
							<input type="checkbox" name="equip[]" value="<?php echo $meResult['name_equipment'];?>">
